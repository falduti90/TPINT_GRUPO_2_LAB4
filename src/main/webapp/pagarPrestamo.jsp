...
<h1 style="margin:auto;text-align:center;margin-bottom:20px;margin-top:30px;">Pagar prestamos</h1>
		<%
		...
		<select class="select" id="select-prestamo mt-2">
			<option value="-1">Seleccione un prestamo</option>
		<% for(Prestamo p : prestamoList){%>
			<option value="<%=p.getCodPrestamo()%>">Codigo: <%=p.getCodPrestamo()%> - Monto: $<%= String.format("%.2f", p.getImporte_a_pagar())%></option>
		
		<%}%> 
		</select>
		<button id="btnElegirPrestamo" class="btnSeleccionar mt-2" onclick="getCuotasPretamo()">Elegir prestamo</button>
	</section>
